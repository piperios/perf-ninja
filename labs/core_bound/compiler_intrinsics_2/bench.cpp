...
static void bench1(benchmark::State& state)
{
    ...
    for (auto& input : inputs)
    {
        ...
        inputContents.emplace_back(std::istreambuf_iterator<char>(inFile), std::istreambuf_iterator<char>());
    }
...
int main(int argc, char** argv)
{
    ...
    if (::benchmark::ReportUnrecognizedArguments(argc - mandatoryArgumentsCount, argv + mandatoryArgumentsCount)) return 1;
