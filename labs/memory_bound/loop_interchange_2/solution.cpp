...
static void
filterVertically(uint8_t* output, uint8_t const* input, int const width, int const height, int const* kernel, int const radius, int const shift)
{
    int const rounding = 1 << (shift - 1);
    ...
    for (int c = 0; c < width; c++)
    {
        ...
        for (int r = radius; r < height - radius; r++)
        {
            // Accumulation
            int dot = 0;
            for (int i = 0; i < radius + 1 + radius; i++) { dot += input[(r - radius + i) * width + c] * kernel[i]; }
...
static void
filterHorizontally(uint8_t* output, uint8_t const* input, int const width, int const height, int const* kernel, int const radius, int const shift)
{
    int const rounding = 1 << (shift - 1);
    ...
    for (int r = 0; r < height; r++)
    {
        ...
        for (int c = radius; c < width - radius; c++)
        {
            // Accumulation
            int dot = 0;
            for (int i = 0; i < radius + 1 + radius; i++) { dot += input[r * width + c - radius + i] * kernel[i]; }
...
// support comments.
bool Grayscale::load(std::string const& filename, int const maxSize)
{
    data.reset();

    std::ifstream input(filename.data(), std::ios_base::in | std::ios_base::binary);
    if (input.is_open())
    {
        std::string line;
        input >> line;
        if (line == "P5")
        {
            ...
            if ((width > 0) && (width <= maxSize) && (height > 0) && (height <= maxSize) && (amplitude >= 0) && (amplitude <= 255) && (c == '\n'))
            {
...
void Grayscale::save(std::string const& filename)
{
    std::ofstream output(filename.data(), std::ios_base::out | std::ios_base::binary);
    if (output.is_open())
    {
        output << "P5" << std::endl << width << ' ' << height << std::endl << "255" << std::endl;
        if (data) { output.write(reinterpret_cast<char const*>(data.get()), size); }
        output.close();
    }
}
