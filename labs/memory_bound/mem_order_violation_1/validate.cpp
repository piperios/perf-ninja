...
static std::array<uint32_t, 256> computeHistogram_original(GrayscaleImage const& image)
{
...
int main(int argc, char** argv)
{
    ...
    std::string const outputGolden = argv[2];
    ...
    if (h1 != h2)
    {
        ...
        for (size_t i = 0; i < h1.size(); i++)
        {
            if (h1[i] != h2[i])
            {
                std::cerr << "Result[" << i << "] = " << static_cast<int>(h1[i]) << ". Expected[" << i << "] = " << static_cast<int>(h2[i])
                          << std::endl;
    ...
    for (size_t i = 0; i < image.size; i++)
    {
        if (p1[i] != p2[i])
        {
            std::cerr << "Result[" << i << "] = " << static_cast<int>(p1[i]) << ". Expected[" << i << "] = " << static_cast<int>(p2[i]) << std::endl;
            if (--downcount <= 0) break;
        }
    }
    return 1;
