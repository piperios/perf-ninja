...
static void reportError(char const* var_name, Received_t received, Expected_t expected, int first_value, int second_value)
{
    std::cerr << "Validation Failed. Value " << var_name << " is " << received << ". Expected is " << expected << " for intialization values "
...
bool check_entry(int first, int second)
{
    ...
    auto const expected_l = static_cast<short>(first * second);
    ...
    auto const expected_d = static_cast<double>(first) / maxRandom;
    ...
    auto const expected_b = (first < second);
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        {
            std::cerr << "Validation Failed. Result[" << i << "].i = " << arr[i].i << ". Expected[" << i << "].i = " << expected[i].i << std::endl;
            return 1;
        }
    }
