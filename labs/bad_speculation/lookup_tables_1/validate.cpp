...
std::array<size_t, NUM_BUCKETS> original_histogram(std::vector<int> const& values)
{
    std::array<size_t, NUM_BUCKETS> retBuckets{0};
    for (auto v : values) { retBuckets[mapToBucket(v)]++; }
    return retBuckets;
}
...
int main()
{
    ...
    if (original_result != result)
    {
        ...
        for (auto v : original_result) std::cerr << v << " ";
        std::cerr << "\n";
