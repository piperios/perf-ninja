...
result_t compute_alignment(std::vector<sequence_t> const& sequences1, std::vector<sequence_t> const& sequences2)
{
    ...
    for (size_t sequence_idx = 0; sequence_idx < sequences1.size(); ++sequence_idx)
    {
        ...
        // since we are only interested in the last value of the last column in the score matrix.
        ...
        for (auto i = 1; i < score_column.size(); ++i)
        {
        ...
        for (auto col = 1; col <= sequence2.size(); ++col)
        {
            ...
            for (auto row = 1; row <= sequence1.size(); ++row)
            {
