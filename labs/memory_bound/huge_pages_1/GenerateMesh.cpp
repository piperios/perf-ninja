...
auto generateMesh(unsigned n_nodes_x, unsigned n_nodes_y, double* x, double* y, unsigned seed) -> std::vector<std::array<unsigned, 2>>
{
    unsigned const n_nodes = n_nodes_x * n_nodes_y;
    ...
    for (unsigned j = 0; j < n_nodes_y - 1; ++j)
    {
        ...
        {
            unsigned const base = n_nodes_x * j + i;
    ...
    for (unsigned i = 0; i < n_nodes_x - 1; ++i)
        topology.push_back(std::array<unsigned, 2>{n_nodes_x * (n_nodes_y - 1) + i, n_nodes_x * (n_nodes_y - 1) + i + 1});
