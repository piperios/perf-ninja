...
std::array<uint32_t, 256> computeHistogram(GrayscaleImage const& image)
{
    ...
    hist.fill(0);
    for (int i = 0; i < image.width * image.height; ++i) hist[image.data[i]]++;
    return hist;
}
...
bool GrayscaleImage::load(std::string const& filename, int const maxSize)
{
    data.reset();
...
// https://people.sc.fsu.edu/~jburkardt/data/pgmb/pgmb.html
void GrayscaleImage::save(std::string const& filename)
{
