...
unsigned original_solution(std::string const& inputContents)
{
...
int main(int argc, char** argv)
{
    ...
    std::string const inputsDirName = argv[1];
    ...
    for (auto& input : inputs)
    {
        ...
        if (!inFile)
        {
            std::cerr << "Validation Failed. No input file: " << inputsDirPath.string() << "\n";
            return 1;
        }
        std::string inputContents{std::istreambuf_iterator<char>(inFile), std::istreambuf_iterator<char>()};
        auto original_result = original_solution(inputContents);
        auto result = solution(inputContents);
        if (original_result != result)
        {
            std::cerr << "Validation Failed on test input: " << input << ". Original result = " << original_result
