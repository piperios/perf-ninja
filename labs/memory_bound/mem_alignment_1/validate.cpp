...
static void original_matmul(float* RESTRICT A, float* RESTRICT B, float* RESTRICT C, int N)
{
    for (int i = 0; i < N; ++i)
        for (int k = 0; k < N; ++k)
...
static bool equals(Matrix const& a, Matrix const& b, int N, int K)
{
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            ...
            if (error >= epsilon)
            {
                std::cerr << "Result[" << i << ", " << j << "] = " << va << ". Expected[" << i << ", " << j << "] = " << vb << std::endl;
...
{
    int const N = 5;
    Matrix a1 = {0.12f, -0.34f, 0.56f, 0.78f, -0.90f, 0.11f, 0.22f, -0.33f, 0.44f,  0.55f,  -0.66f, 0.77f, -0.88f,
                 0.99f, 0.10f,  0.21f, 0.32f, 0.43f,  0.54f, 0.65f, -0.12f, -0.23f, -0.34f, -0.45f, 0.56f};

    Matrix b1 = {0.45f,  -0.55f, 0.65f,  -0.75f, 0.85f,  -0.15f, 0.25f,  -0.35f, 0.45f,  -0.55f, 0.85f,  -0.75f, 0.65f,
                 -0.55f, 0.45f,  -0.25f, 0.35f,  -0.45f, 0.55f,  -0.65f, 0.55f,  -0.45f, 0.35f,  -0.25f, 0.15f};

    Matrix expected = {-0.109f, 0.107f,  -0.105f, 0.103f,   -0.101f, -0.0715f, 0.1485f,  -0.2255f, 0.3025f, -0.3795f, -1.353f, 1.517f, -1.681f,
                       1.845f,  -2.009f, 0.6345f, -0.4615f, 0.2885f, -0.1155f, -0.0575f, 0.112f,   -0.146f, 0.18f,    -0.214f, 0.248f};

    Matrix c1;
    c1.resize(N * N);
    initZero(c1, N, N);
    original_matmul(a1.data(), b1.data(), c1.data(), N);
    if (!equals(c1, expected, N, N)) return 1;

    int const K = n_columns(N);
    Matrix a2;
    a2.resize(N * K);
    copyFromMatrix(a1, a2, N, K);
    Matrix b2;
    b2.resize(N * K);
    copyFromMatrix(b1, b2, N, K);
    Matrix c2;
    c2.resize(N * K);
    initZero(c2, N, K);

    interchanged_matmul(a2.data(), b2.data(), c2.data(), N, K);
    if (!equals(c2, expected, N, K)) return 1;
