...
uint32_t original_solution(char const* file_name)
{
    std::fstream file_stream{file_name};
    if (!file_stream.is_open()) throw std::runtime_error{"The file could not be opened"};
...
int main()
{
    auto const original_result = original_solution(small_data);
    auto const result = solution(small_data);
