...
public:
    ...
    explicit inline MappedFile(std::string const& path);
...
{
    int const fd = open(path.c_str(), O_RDONLY | O_LARGEFILE);
    if (fd == -1) throw std::runtime_error{"Could not open the specified file"};
    struct stat sb{};
    if (fstat(fd, &sb)) throw std::runtime_error{"Could not obtain file info"};
    ...
    if (mapped_addr == MAP_FAILED) throw std::runtime_error{"Could not mmap file"};

    m_content = static_cast<char*>(mapped_addr);

    if (close(fd)) throw std::runtime_error{"Could not close file after mmap"};
}
...
MappedFile::MappedFile(const std::string& path)
{
    ...
    if (m_file_handle == INVALID_HANDLE_VALUE) throw std::runtime_error{"Could not open file"};
    ...
    auto const map_content = MapViewOfFile(m_mapping_handle, FILE_MAP_READ, 0, 0, 0);
    if (map_content == NULL)
    {
...
MappedFile::MappedFile(MappedFile&& other) noexcept
    : m_content{other.m_content}, m_size{other.m_size}, m_file_handle{other.m_file_handle}, m_mapping_handle{other.m_mapping_handle}
{
    other.m_content = nullptr;
}
